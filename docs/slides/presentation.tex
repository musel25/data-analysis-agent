...
\begin{enumerate}\footnotesize
  \item A \textbf{Docker container} is provisioned: Python, the scientific stack, the data files staged on disk.
  \item A \textbf{harness} --- the agent scaffold --- drives the loop: it sends the prompt to the model, \textbf{executes the code the model emits}, feeds stdout back, manages context. \sm{The model never touches the machine.}
  \item The model gets the question \textbf{at a high level}, and is \emph{deliberately not} told the method: which rows to keep, which estimator, when the evidence suffices.
  \item It writes and runs its own code, in a loop, reading its own output. \textbf{No human, no hints.}
...
\midrule
\textbf{Harness} & One, \textbf{fixed}; never ablated & \gd{\textbf{Six} --- and it is a \emph{variable}} \\
\textbf{Tools}   & Plain scientific stack \sm{($+$ PLINK)} & \textbf{BIOMNI} \sm{--- 226 importable functions} \\
\textbf{Internet}& \gd{Off} & \rd{On} \\
\bottomrule
\end{tabular}
\src{GBP Methods, p.\,15; DDB \S3--4, pp.\,6--7}

\pause
\raggedright
\begin{alertblock}{So ``the model scored $X$'' means ``the model, \emph{in this harness}, scored $X$''}
Not pedantry --- DDB measures it: swapping the harness moves one model by \textbf{11 points}, more than the
gap between the \#1 and \#2 \emph{models}. \textbf{GBP ranks 60 configurations on one fixed harness.}
\end{alertblock}
\end{frame}

\begin{frame}{The two papers, and how to read a benchmark number}
\why{What a ``pass rate'' actually is, mechanically. Both papers work the same way; only the knobs differ.}

\begin{block}{The machinery, in five lines}
\begin{itemize}
  \item A \textbf{benchmark} $=$ a fixed set of problems $+$ an automatic grader.
  \item One \textbf{attempt} $=$ one (configuration, problem) run $\to$ \textbf{pass or fail}. \textbf{Binary}: all graded fields right, or the run is a failure. No partial credit.
  \item Agents are \textbf{stochastic}, so every pair is run repeatedly: \textbf{GBP 10 attempts} per (model, problem); \textbf{DDB 3 trials} per (setting, task).
  \item \textbf{Per-problem pass rate} $=$ attempts passed $/$ attempts run \sm{(GBP: $k/10$)}. \textbf{Headline} $=$ the mean of that over all problems.
  \item A \textbf{configuration / setting} is \emph{not} just a model. GBP: model $\times$ \emph{reasoning effort} $\Rightarrow$ \textbf{60 rows} from $\sim$20 models. \sm{GPT-5.6 Sol at none/low/medium/high/xhigh/max is six separate rows.} DDB: model $\times$ \emph{harness} $\times$ effort $\Rightarrow$ \textbf{29 settings}.
\end{itemize}
\end{block}
\pause
\smallskip
\begin{itemize}
  \item \textbf{GeneBench-Pro} \sm{(Li \& Ho, OpenAI --- bioRxiv, 30 June 2026)}: \textbf{129 problems}, genomics and translational biomedicine. Graded by \textbf{deterministic Python}.
  \item \textbf{DrugDiscoveryBench} \sm{(Aky\"urek, Tu et al., Scale AI \& Phylo, 2026)}: \textbf{82 tasks}, early-stage drug discovery. Graded by an \textbf{LLM judge} against expert rubrics.
\end{itemize}
...
\why{The motivation is economic. Both papers reach for the same argument.}

\begin{itemize}
  \item \textbf{The bottleneck moved.} Sequencing got cheap; the analysis did not.
        \smallskip
        \sm{``as sequencing has scaled, downstream computation and analysis, rather than data generation,
        have become the central scientific bottleneck.'' --- GBP, p.\,2}
  \pause
  \medskip
  ...
  \pause
  \medskip
  \item \textbf{And the step is slow and expensive.} One GBP problem costs a human expert
        \textbf{10--40 hours} at \$100--200/hr --- \emph{``on the order of a few thousand dollars''}.
        \sm{--- GBP, p.\,14 (their own illustrative figure)}
\end{itemize}
\pause
\vfill
...
\pause
\smallskip
\centering\sm{Note the consequence: a benchmark can be hard because the arithmetic is fiddly --- and that
tells you \emph{nothing}. These are hard at the \textbf{judgement calls}, which is what makes them worth
studying.}
\end{frame}

\begin{frame}{The \emph{decision point} --- GBP's central construct}
\why{The unit these problems are built out of, and the one idea I took wholesale.}

\begin{block}{A \emph{decision point}}
\begin{quote}\itshape
...
\medskip
\begin{itemize}
  \item Every GBP problem has $\sim$\textbf{six places to take a defensible-looking wrong turn} --- and the wrong turn lands the answer somewhere \emph{measurably} else.
...
\begin{itemize}
  ...
  \item \textbf{Fully simulated} data --- so the causal structure is \emph{known}, and nothing can be memorised from the literature.
  ...
  \item \textbf{60 configurations} \sm{(model $\times$ reasoning effort)}, \textbf{10 attempts} each per problem \sm{(5 for the GPT Pro (Extended) and Claude Opus rows --- 11 of the 60)}.
  \item 82 of 129 problems reviewed by \textbf{11 external domain experts}.
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{block}{What ``simulated'' buys them}
\begin{enumerate}\footnotesize
...
\end{block}
\smallskip
\sm{The staged files are built to look like what an analyst actually receives: batch effects, sentinel
values, missingness, mixed ancestry --- the defects are \emph{deliberate}, and they are the point.}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{The subtlest design choice in either paper --- \emph{what do you grade against?}}
\why{Easy to skim past. It is the difference between a benchmark that works and one that is broken.}

You simulated the data, so you know the true parameter you dialled in. \textbf{Grade against that?}
\textbf{No} --- and this is the whole point.
...
\footnotesize
Simulator knob: true effect $\beta = \mathbf{0.30}$.
\smallskip

But the staged files are a \emph{finite sample} drawn from it. In \emph{those files}, the effect a
\textbf{flawless} analyst recovers is $\hat\beta = \mathbf{0.283}$ --- sampling noise.
\smallskip

Grade against $0.30$, tolerance $\pm 0.01$:
\smallskip

\centering\rd{a perfect analysis is marked WRONG.}
\end{alertblock}
\end{column}
\begin{column}{0.48\textwidth}
...
The target is the \textbf{recoverable estimand}, not the generating parameter.
\smallskip
...
\pause
\medskip
\begin{block}{And two more guards that make the tolerances mean something}
\begin{itemize}\footnotesize
  \item \textbf{Ablation-verify the separation:} for every trap, assert the plausible-but-\emph{wrong} answer falls \emph{far outside} the tolerance band. \sm{Otherwise the lazy analysis lands inside the band, is graded correct, and the benchmark measures nothing.}
  \item \textbf{Robust to defensible variation:} \emph{``nearby reasonable thresholds lead to the same graded outcome''} (p.\,6). \sm{Otherwise you are measuring luck at the cutoff lottery.}
\end{itemize}
\end{block}
\src{GeneBench-Pro, Table 1, p.\,6 --- which names this exact failure mode. Subtle, and right.}
\end{frame}
...
GPT-5.6 Luna (max)                                   & 16.5\% & [11.6, 21.7] & 64.3\% \\
...
\bottomrule
\end{tabular}
\src{Supplementary Table 1, p.\,21 --- the last column is a \emph{separate column} in their table, not derivable from the pass rate.}

\pause
\medskip
\begin{alertblock}{The number that should stop you --- and it is not 28.7\%}
\textbf{28.7\%} is an \emph{average over problems}. Read casually, it sounds like ``passes about 29\% of
the time, on everything''. \textbf{It does not.}
\medskip

The best mainline model scores \rd{zero across all ten attempts on 45.7\% of problems} --- nearly half
the benchmark is \textbf{never solved once, in ten tries}. \textbf{The score is concentrated in the
problems it can already do}, and there is a large tail it simply cannot touch.
\end{alertblock}
\end{frame}

\begin{frame}{How to read those two columns}
\why{Two things the table is quietly telling you. Both matter more than the ranking.}

\begin{block}{1. The 95\% CI --- and what a ``resample'' is}
The 129 problems are only \emph{a sample} of the problems they could have written, and each run is
noisy. So the CI asks: \textbf{if we redrew the problem set and re-ran everything, where would the
headline land?}
\medskip

They answer it by \textbf{hierarchical bootstrap}: draw a fake problem set by sampling the 129 problems
\emph{with replacement}, then resample the runs within each drawn problem, recompute the mean --- and do
that \textbf{20,000 times}. Those 20,000 synthetic redraws are the \emph{resamples}. The CI is the middle
95\% of the resulting spread.
\medskip

\centering\rd{The bars are $\sim\pm 6$ points wide. So \textbf{28.7\% vs 31.5\% is not a real gap.}}
\end{block}
\pause
...
buys only \textbf{1.9 points}. You cannot spend your way out of this benchmark.}
\end{block}
\end{frame}
...
\begin{column}{0.55\textwidth}
\begin{itemize}
  \item \textbf{82 tasks written by pharmaceutical scientists}, spanning the four early-discovery stages: target ID $\to$ hit ID $\to$ hit-to-lead $\to$ lead optimisation.
  \item Each task is anchored to \textbf{one specific real artifact} --- \sm{a granted patent, a PDB entry, a paper, a database record (UniProt / ChEMBL / OpenTargets), or a file shipped in the container.} \textbf{The answer is checkable against that object.}
  \item Environment: \textbf{BIOMNI} \sm{(Stanford SNAP lab's biomedical agent framework)}, exposed to the agent as \textbf{an importable Python library}:
        ...
        \smallskip
        226 functions, 22 domains, a 76-file data lake. \textbf{Internet on.} 120 min/task.
  \item Answer written to \code{/workspace/answer.md}; graded by an \textbf{LLM judge} (GPT-5.4) against the author's weighted rubric. \textbf{Pass $=$ 100\% of \emph{outcome} criteria.}
\end{itemize}
\end{column}
\begin{column}{0.45\textwidth}
\begin{block}{What they vary --- 29 settings, 3 trials each}
\footnotesize
\textbf{Model} --- 12 frontier, incl. open-weight.\\
\textbf{Harness} --- 6 \sm{(native where it exists: Codex, Claude Code, Gemini CLI; else mini-SWE-agent, Pi, OpenCode)}.\\
\textbf{Reasoning effort} --- low / medium / \emph{ceiling} \sm{(swept for GPT-5.5, Opus 4.8, Gemini 3.5 Flash)}.
\end{block}
\pause
...
Agent must fetch both PDB entries, superimpose, find the residue whose conformation shifts, verify it
contacts compound B. \textbf{Answer: \code{THR58}.}
\end{exampleblock}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{How a DDB task was built --- and the rule that will come back to bite them}
...
\begin{enumerate}\footnotesize
  \item A \textbf{domain expert authors} a candidate task, its answer, and a \textbf{playbook} --- a step-by-step guide to the solution.
  \item A \textbf{second expert reviews} and edits it.
  \item \textbf{BenchGuard} \sm{(automated)} --- checks prompt/answer fidelity, flags ambiguous wording. \hfill\sm{$\Big\}$ run in parallel}
  \item \textbf{Solvability assessment} --- hand the agent the \textbf{playbook as a hint}, and confirm it \emph{can} reach the answer when given the detailed steps.
  \item An \textbf{SME reviews} whatever gates 3--4 surfaced and fixes it --- \rd{\textbf{dropping the task entirely if it cannot be made robust}}. $\leftarrow$ \textbf{the drop rule}
  \item An \textbf{independent QC team} signs off before release.
\end{enumerate}
\end{block}
\pause
\medskip
\begin{alertblock}{Gates 4 and 5 together are an \emph{inclusion criterion}}
A task only enters the benchmark if an agent \textbf{can} solve it \emph{when handed the expert's playbook}.
\medskip

\centering\textbf{Hold on to that.} \sm{It quietly determines what the paper's headline finding can possibly show ---
and I come back to it in the critique.}
\end{alertblock}
\end{frame}

\begin{frame}{DrugDiscoveryBench --- the results}
\begin{columns}[T]
...
\raggedright\sm{Gemini 3.5 Flash (Gemini CLI) 50.0\%; Opus 4.8 (mini-SWE-agent) 46.8\%. Open-weight trails
sharply: GLM 5.2 37.8\%, Kimi 2.7 35.3\%.}
\end{block}
\end{column}
...
GPT-5.5 swings \textbf{40.6 $\to$ 51.6\%} across harnesses --- an \textbf{11-point spread}. The gap between
...
\pause
\medskip
\begin{alertblock}{Finding 2 is the one most papers cannot report --- and it indicts GBP}
The \textbf{scaffold} is a first-class experimental variable here, and it turns out to matter
\textbf{more than the choice of model}.
\medskip

\centering\textbf{GBP ranks 60 configurations on a single, fixed, unablated harness.}
\end{alertblock}
\end{frame}
...
\begin{block}{The four principles, recapped \sm{--- all of them from the design slide earlier}}
\begin{enumerate}\footnotesize
  \item \textbf{Simulate, so you can \emph{compute} the truth} --- and so you can \emph{plant} a decision point and be certain the naive path lands elsewhere.
  \item \textbf{Grade the recoverable estimate}, not the generating parameter.
  \item \textbf{Ablation-verify the separation} --- the wrong answer must land \emph{far outside} the tolerance band.
  \item \textbf{Be robust to defensible analyst variation} --- nearby reasonable thresholds must grade the same.
\end{enumerate}
\end{block}
\pause
\medskip
\begin{alertblock}{And the fifth --- \textbf{binary, all-or-nothing grading}, which they defend better than I could}
\begin{quote}\itshape
``an agent that executes several intermediate steps correctly but returns the wrong
decision-relevant answer \textbf{has not successfully automated the analysis}.'' \src{(p.\,14)}
\end{quote}
\centering\medskip
\textbf{Partial credit measures effort. Binary measures usefulness.}
\end{alertblock}
\pause
\medskip
\centering\sm{Together these are why I trust GBP's \emph{numbers} --- whatever I go on to say about its
\emph{story}.}
\end{frame}
