...
\begin{itemize}
  \item What they address, and why it is worth a benchmark
  \item How you even build one: \textbf{construction, validation, grading}
  \item What they found --- the numbers, and what they mean
  \item \textbf{How these agents actually fail} --- the takeaways
  \item The limitations, and the one that grounds my design
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{frame}{The opportunity --- and why it is worth measuring}
\why{AI agents can now write and run code in a loop, unsupervised. Both papers ask the same next question: can they do the part of science that is \emph{judgement}, not execution?}
...
\begin{itemize}
  ...
  \item \textbf{And the step is slow and expensive.} One GBP problem is \textbf{10--40 hours} of expert
        time at \$100--200/hr --- \emph{``on the order of a few thousand dollars''}. \sm{--- GBP, p.\,14}
\end{itemize}
\pause
\vfill
...
\begin{frame}{Why it is hard to measure --- judgement, not execution}
\why{GBP's Figure 1 is the whole argument for why these papers exist. A benchmark can be hard for the wrong reason.}
...
data can support, what data to include, which estimand or model is appropriate\ldots{} and when the
evidence is strong enough to support a conclusion.''
\end{quote}
\src{GeneBench-Pro, p.\,2 --- verbatim}
\pause
\smallskip
\centering\sm{So a benchmark that is hard because the arithmetic is fiddly tells you \emph{nothing}.
These are hard at the \textbf{judgement calls} --- which is exactly what makes them worth studying.}
\end{frame}

\begin{frame}{The system under evaluation --- and the two papers}
\why{Neither paper evaluates a bare LLM. What is measured is a \textbf{model $+$ a harness}, inside a container.}

\begin{block}{What is actually run, per task}
...
\begin{itemize}\footnotesize
  \item \textbf{GeneBench-Pro} \sm{(Li \& Ho, \textbf{OpenAI} --- bioRxiv, 30 June 2026)}: \textbf{129 problems}, genomics and translational biomedicine. \gd{Graded by deterministic Python.}
...
\end{itemize}
\pause
\begin{alertblock}{One number to keep in your pocket: ``the model scored $X$'' means ``\emph{in this harness}, $X$''}
DDB measures it --- swapping the harness moves one model \textbf{11 points}, more than the gap between
the \#1 and \#2 \emph{models}. \sm{Both are preprints: GBP ``not certified by peer review''; DDB prints no venue.}
\end{alertblock}
\end{frame}
...
\begin{frame}{GeneBench-Pro --- what it is, and the one idea to keep}
...
And the errors \textbf{cascade}: \emph{``an incorrect choice at any stage propagates into downstream
errors and\ldots{} failure to recover the final correct target''} (p.\,5). \textbf{This is the idea I
took wholesale for Part 2}: plant the fork, know where the wrong path lands --- then you can say not
just \emph{that} the agent was wrong, but \textbf{which wrong}.
\end{alertblock}
\end{frame}

\begin{frame}{First, the intuition --- every benchmark like this answers three questions}
\why{Before the numbers. Building a benchmark that measures \emph{judgement} forces three design choices --- and the two papers answer them very differently.}

\begin{block}{}
\begin{enumerate}
  \item[\textbf{1.}] \textbf{Construction.} How do you build tasks that test \emph{judgement}, not execution --- and whose true answer you actually know?
  \item[\textbf{2.}] \textbf{Validation.} How do you \emph{guarantee} that a plausible-but-wrong path scores wrong, and a correct path scores right? \sm{(Otherwise the benchmark measures nothing.)}
  \item[\textbf{3.}] \textbf{Grading.} How do you score an open-ended analysis \textbf{automatically}, with no human in the loop?
\end{enumerate}
\end{block}
\pause
\vfill
\begin{alertblock}{GeneBench-Pro answers all three with one decision: \emph{simulate the data}}
Everything clever about GBP falls out of that single choice. \sm{DDB, in the next section, makes the
opposite choice --- real data --- and has to answer the same three questions the hard way.}
\end{alertblock}
\end{frame}

\begin{frame}{GBP --- Construction \& Validation: simulate, so you can \emph{compute} the truth}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{block}{Construction --- what simulation buys}
\begin{enumerate}\footnotesize
  \item You can \textbf{compute} the truth instead of assuming it.
  \item You can \textbf{plant} a decision point and \emph{prove} the naive path lands elsewhere.
  \item No model can have seen the answer.
\end{enumerate}
\end{block}
\end{column}
\begin{column}{0.5\textwidth}
\begin{block}{Validation --- making the trap mean something}
\begin{itemize}\footnotesize
  \item \textbf{Ablation-verify the separation:} for every trap, \emph{assert} the plausible-but-wrong answer falls \textbf{far outside} the tolerance band.
  \item \textbf{Robust to defensible variation:} \emph{``nearby reasonable thresholds lead to the same graded outcome''} (p.\,6).
\end{itemize}
\end{block}
\end{column}
\end{columns}
\pause
\medskip
\begin{alertblock}{Why validation is not optional}
Without the separation assertion, a \textbf{lazy analysis lands inside the tolerance band, is graded
correct, and the benchmark measures nothing.} \sm{I implemented this exact guard in Part 2 --- and it
immediately killed two of my own tasks whose wrong path landed \emph{inside} the band. Both looked
perfectly reasonable. Both graded nothing. The assertion found them; I did not.}
\end{alertblock}
\end{frame}
...
$\mathbf{0.283}$. The target is the \textbf{recoverable estimand}, not the generating parameter.
\smallskip
...
\midrule
GPT-5.6 Sol Pro (Extended) --- \emph{best in paper} & \textbf{31.5\%} & [24.3, 38.9] & 55.8\% \\
GPT-5.6 Sol (max) --- \emph{best mainline}          & \textbf{28.7\%} & [22.5, 35.1] & \rd{45.7\%} \\
GPT-5.6 Terra (max)                                  & 23.3\% & [17.8, 29.2] & 49.6\% \\
Claude Opus 4.8 (max) --- \emph{strongest non-GPT}   & 16.0\% & [11.0, 21.2] & 67.4\% \\
GPT-5.5 (xhigh)                                      & 12.0\% & [8.2, 16.1]  & 64.3\% \\
\midrule
\multicolumn{4}{@{}l@{}}{\sm{Non-GPT range across all rows: \textbf{0.6\%} (MiniMax M2.7) to 16.0\% (Claude Opus 4.8)}}\\
\bottomrule
\end{tabular}
\src{Supplementary Table 1, p.\,21 --- the last column is separate, not derivable from the pass rate.}
\pause
\medskip
...
tries}. The score is concentrated in the problems it can \emph{already} do; there is a large tail it
simply cannot touch.
\end{alertblock}
\end{frame}

\begin{frame}{GBP --- how to read those numbers}
\why{Two things the table is quietly telling you. Both matter more than the ranking.}

\begin{block}{1. The 95\% CI --- so \textbf{28.7\% vs 31.5\% is not a real gap}}
The 129 problems are only \emph{a sample}, and each run is noisy. GBP asks: if we redrew the problem set
and re-ran everything, where would the headline land? They answer by \textbf{hierarchical bootstrap} ---
resample the 129 problems \emph{with replacement}, resample the runs within each, recompute the mean,
\textbf{20,000 times}. The middle 95\% is the interval.
\smallskip

\centering\rd{The bars are $\sim\pm 6$ points wide. The top three rows are a statistical tie.}
\end{block}
\pause
\begin{block}{2. Reasoning effort buys a lot --- then saturates}
\centering
GPT-5.6 Sol, \emph{none} $\to$ \emph{max}:\quad
3.7 $\to$ 14.4 $\to$ 22.5 $\to$ 24.4 $\to$ 26.8 $\to$ \textbf{28.7\%}
\smallskip

\raggedright\sm{An \textbf{8$\times$} gain from inference compute alone --- but \emph{high}$\to$\emph{max}
buys only \textbf{1.9 points}. \textbf{You cannot spend your way out of this benchmark.}}
\end{block}
\end{frame}

\begin{frame}{GBP --- limitations}
\why{Three, and the first is structural. I trust GBP's \emph{numbers}; I am warier of its \emph{story}.}
...
\begin{itemize}
  \item \rd{\textbf{OpenAI is evaluating OpenAI's own models.}} The leaderboard is topped by GPT-5.6
        ...
        headline model and the framing are in-house}, and the promised third-party check \sm{(a 50-task
        subset to Artificial Analysis)} \textbf{is not in the paper}.
  \pause
  \item \rd{\textbf{The central ``notice--act'' story has no $n$.}} It rests on manual review of
        \emph{``selected''} GPT-5.5 / 5.6 comparisons plus the model's own \textbf{ungraded self-report}
        --- never checked against what its code actually did. \sm{The pass rates are solid; the mechanism
        is an anecdote with $n$ unstated.}
  \pause
  \item \rd{\textbf{The error bars degenerate exactly where the argument lives.}} Supplementary Table 2
        prints \textbf{$0.0 \pm 0.0\%$} for eight rows --- a zero-width 95\% CI is the bootstrap dying on
        all-zero data, \emph{tightest precisely where the data carries no information}.
\end{itemize}
\pause
\medskip
\begin{alertblock}{Two more, briefly}
\sm{$\bullet$ Most rows use \textbf{10 attempts}, eleven use \textbf{5} --- which mechanically inflates
the ``never solved'' statistic for those rows, so the columns are not comparable.\quad
$\bullet$ \textbf{No small models, no human baseline}: 28.7\% has no ceiling to be read against. Is an
expert 90\%? 60\%? The paper does not say.}
\end{alertblock}
\end{frame}
...
\begin{frame}{DrugDiscoveryBench --- what early drug discovery actually is}
\why{The second paper trades GBP's simulated certainty for real-world grounding. To read its numbers, you need to know what its tasks \emph{are}.}
...
\raggedright\sm{Each stage mixes database mining, structural biology, and cheminformatics --- and a
wrong call early is expensive: most programmes that enter never reach the clinic, and targets with
\textbf{human genetic evidence are \emph{``$\sim$2.6$\times$ more likely to succeed''}} (p.\,2). Getting
the early analysis right is where the value is.}
\end{block}
\pause
\medskip
...
\sm{a granted patent, a PDB structure, a published paper, or a database record (UniProt / ChEMBL /
OpenTargets).} The answer is checkable against that object.
\medskip

\centering\textbf{Where GBP \emph{simulates} to own the truth, DDB \emph{pays experts} to author it.}
\sm{ 82 expert tasks on real patents is the part you cannot generate.}
\end{alertblock}
\end{frame}

\begin{frame}{DDB --- Construction: real artifacts, and a 226-function toolbox}
\begin{columns}[T]
\begin{column}{0.54\textwidth}
\begin{itemize}\footnotesize
  \item Tasks span all four stages, target ID $\to$ lead optimisation.
  \item Environment: \textbf{BIOMNI} \sm{(Stanford SNAP lab's biomedical agent framework)}, exposed to the agent as \textbf{an importable Python library}:
        \smallskip
        \sm{\code{from biomni.tool.database import query\_uniprot}}
        \smallskip
        \textbf{226 functions, 22 domains, a 76-file data lake. Internet on. 120 min/task.}
  \item Answer written to \code{/workspace/answer.md}.
\end{itemize}
\end{column}
\begin{column}{0.46\textwidth}
\begin{exampleblock}{A sample task \sm{(lead optimisation)}}
\footnotesize
...
and verify it contacts compound B. \textbf{Answer: \code{THR58}.}
\end{exampleblock}
\end{column}
\end{columns}
\pause
...
\begin{frame}{DDB --- Validation: the authoring pipeline, and the rule that bites}
\why{Six gates. Gate 4 is the one to remember --- it decides which tasks are \emph{allowed} into the benchmark.}

\begin{block}{The authoring pipeline \sm{(DDB, Figure 2 and \S2.2, p.\,5)}}
\begin{enumerate}\footnotesize
  \item A \textbf{domain expert authors} a task, its answer, and a \textbf{playbook} --- a step-by-step guide to the solution.
  \item A \textbf{second expert reviews} and edits it.
  \item \textbf{BenchGuard} \sm{(automated)} checks prompt/answer fidelity and flags ambiguity.
  \item \textbf{Solvability assessment} --- hand the agent the \textbf{playbook as a hint}, and confirm it \emph{can} reach the answer.
  \item An \textbf{SME} fixes whatever gates 3--4 surfaced --- \rd{\textbf{dropping the task entirely if it cannot be made robust.}}
  \item An \textbf{independent QC team} signs off before release.
\end{enumerate}
\end{block}
\pause
\medskip
...
playbook}. \sm{It quietly determines what the paper's headline finding can possibly show --- and it
comes back in the limitations.}
\end{alertblock}
\end{frame}

\begin{frame}{DDB --- Grading, and the results}
\why{Where GBP had deterministic Python, DDB must grade open-ended answers on real data --- so it needs a judge.}

\begin{columns}[T]
\begin{column}{0.44\textwidth}
\begin{block}{Grading}
\footnotesize
\code{answer.md} $\to$ an \textbf{LLM judge (GPT-5.4)} against the author's \textbf{weighted rubric}.
\textbf{Pass $=$ 100\% of the \emph{outcome} criteria.}
\end{block}
\begin{block}{Best agent: \textbf{51.6\%}}
\centering\footnotesize
GPT-5.5 $+$ mini-SWE-agent (xhigh)\\
\raggedright\sm{Gemini CLI 50.0\% (\#2); Opus 4.8 (Claude Code) 45.1\%. Open-weight trails: GLM 5.2 37.8\%.}
\end{block}
\end{column}
...
GPT-5.5 swings \textbf{40.6 $\to$ 51.6\%} across harnesses --- an \textbf{11-point spread}, versus a
\textbf{1.6-point} gap between the \#1 and \#2 \emph{models}.
\end{block}
\end{column}
\end{columns}
\pause
\medskip
\begin{alertblock}{Finding 2 indicts GBP --- and a warning}
The scaffold matters \emph{more than the model}, yet \textbf{GBP ranks 60 configurations on one fixed,
unablated harness}. And do \emph{not} compare 51.6\% to GBP's 28.7\%: DDB's tasks were
\textbf{pre-screened to be solvable}. Different scales.
\end{alertblock}
\end{frame}

\begin{frame}{DDB --- limitations}
\why{The scores flow through a judge, the taxonomy through single runs, and the headline through an inclusion criterion.}

\begin{itemize}
  \item \rd{\textbf{The judge has never been shown a human.}} Every number flows through \textbf{one LLM
        judge (GPT-5.4)}; the only check is against \emph{two other LLMs} ($\kappa = 1.0$). Perfect
        agreement is \emph{suspicious}, not reassuring --- and the default judge is the \textbf{same
        family} as the leaderboard winner (GPT-5.5). \sm{The obvious missing experiment: have an expert grade 50.}
  \pause
  \item \rd{\textbf{The 54\% failure taxonomy is one run per cell.}} Table 3's own caption: \emph{``one
        run per (model, task)\ldots{} 226 model-failure runs''} --- no repeats, no error bar, in a paper
        whose own results show agent runs are enormously noisy.
  \pause
  \item \rd{\textbf{The headline hint result is near-circular.}} ``Give agents the playbook and 80/82
        pass'' --- but \textbf{solvability-given-the-playbook was an \emph{inclusion criterion}} (Gate 4).
        The benchmark contains, by construction, tasks solvable with the playbook. \sm{And ``80/82'' is an
        \emph{oracle}: 76/82 already fall to best-of-$\sim$87 runs, the playbook lifts only the last 6. The
        best single agent is 51.6\%.}
\end{itemize}
\pause
\smallskip
...
% ###########################################################################
\section{How these agents fail --- and what it means for a design}
% ###########################################################################

\begin{frame}{The takeaway that grounds Part 2 --- over half of failures are not coding failures}
\why{DDB classified 226 failing runs into root causes. This table is what a builder actually needs.}
...
was right there. \sm{I attack this table's \emph{methodology} in the limitations --- but qualitatively it
is the most useful artifact in either paper, so I reuse its five categories verbatim. The next five
slides make each one concrete.}
\end{alertblock}
\end{frame}
...
\begin{frame}{Two labs, two domains, two grading philosophies --- one diagnosis}
\begin{block}{}
\begin{quote}\itshape
``their failures are seldom about missing biological facts, more often they involve \textbf{losing the
scientific thread of the task}\ldots{} A central missing capability is\ldots{} \textbf{keeping the
original scientific objective active while executing many local tool calls and intermediate
decisions}.''
\end{quote}
...
\pause
\medskip
\begin{alertblock}{The conclusion both papers earn --- and it is good news for a builder}
\begin{quote}\itshape
``\textbf{execution is within reach for today's agents should they be given the expert workflow.}'' \src{(DDB, p.\,14)}
\end{quote}
\centering\medskip
The bottleneck is not code generation, not knowledge, not context length. It is \textbf{procedure} ---
and \textbf{procedure is something an engineer can build.} \sm{(That is the entire thesis of Part 2.)}
\end{alertblock}
\end{frame}

\begin{frame}{The limitation neither paper states --- both diagnoses live at the frontier}
\why{The limitation that matters most to me, because I am the reader they did not write for.}

\begin{block}{Every diagnosis is measured on frontier models only}
\footnotesize
\textbf{GBP:} all \textbf{60 configurations} are frontier or near-frontier --- no small, cheap, or
distilled tier, not one. \quad
\textbf{DDB:} the failure taxonomy runs on \textbf{six models, all closed, all frontier} (p.\,11);
\emph{zero} open-weight models appear in it.
\end{block}
\pause
\medskip
\begin{alertblock}{And GBP silently promotes a \emph{relative} claim to an \emph{absolute} one}
\footnotesize
\textbf{Results, p.\,3} (a comparison of two strong models): \emph{``the main qualitative improvement
\textbf{in stronger models} lies\ldots{} in turning those observations into concrete corrective\ldots{}
decisions.''}\\
\smallskip
\textbf{Discussion, p.\,14} (now a property of the class): \emph{``\textbf{frontier models consistently
notice} data issues\ldots{} there remains an incomplete ability to bridge the `notice--act' gap.''}
\end{alertblock}
\pause
\smallskip
\begin{block}{}
\centering\footnotesize
And GBP's \emph{own} data argue against extending it downward: GPT-5.2 (none) scores \textbf{0.5\%}, with
\rd{96.9\%} of problems never solved. \textbf{``Notices but fails to act'' is not a credible description
of a system that never once solves 96\% of the suite} --- far likelier, it is not noticing at all.
\end{block}
\end{frame}
...
\medskip
\begin{itemize}\footnotesize
  \item Most people building agents \textbf{will not be running the frontier}. A benchmark that diagnoses
        a bottleneck \textbf{owes its readers the capability range over which it holds} --- the cheapest
        experiment in either paper, and the one neither ran.
\end{itemize}
\pause
\medskip
\centering
\textbf{I built my agent on a small, cheap model --- so I got to find out.}\\
\sm{And it \textbf{inverted my own design thesis}. That is where Part 2 begins.}
\end{frame}
